...
export default function ImageSlider() {
  const [visibleImages] = useState(images.slice(0, 2));
  ...
  return (
    ...
    /*
    <div className="relative w-full flex justify-center items-center py-6 object-cover">  {/* bg-gray-100 *//*}
      <div className="flex space-x-14 screen-full ">
        <AnimatePresence>
          {visibleImages.map((img, index) => (
            <motion.img
              ...
              src={img}
              alt={`Slide ${index}`}
              className="w-1/2 h-auto rounded-lg shadow-lg overflow-hidden"
